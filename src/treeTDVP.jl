...
function mpsrightnorm!(net::TreeNetwork, id::Int)
    ...
    for (i, child) in enumerate(children)
        ...
        dpar = size(net[id])
        C, AR = lq(CuArray(reshape(net[child], dchild[1], :)))
        #EDIT
        net[child] = reshape(CuMatrix(AR), dchild)
        IC=collect(1:nc+2)
        IA=collect(1:nc+2)
        IC[i+1]=-1
...
function tdvp1sweep!(dt, A::TreeNetwork, M::TreeNetwork, F::Vector, id::Int; verbose=false, kwargs...)
    ...
    AC, info = exponentiate(x->applyH1(x, M_id, F0, F_children...), -im*dt/2, AC; ishermitian=true)
    if verbose
    ...
    for (i, child) in enumerate(children)
        ...
        F_id = F[id] #EDIT
        F_child = F[child]
        C, info = exponentiate(x->applyH0(x, F_id, F_child), im*dt/2, C; ishermitian=true)
        if verbose
        ...
        #evolve C backwards half a time step
        F_child = F[child] #EDIT
        F_id = F[id]
        C, info = exponentiate(x->applyH0(x, F_child, F_id), im*dt/2, C; ishermitian=true)
        if verbose
